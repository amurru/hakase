// task_cli.go - the `hakase task` CLI: create, list, get, update, delete, claim, complete, fail, cancel, summary
package main
...
func runTaskList(args []string) int {
	...
	for _, task := range tasks {
		prioritySymbol := prioritySymbol(task.Priority)
		statusSymbol := statusSymbol(task.Status)
		depsStr := ""
		...
		fmt.Printf("  %s %s %s%s%s\n", statusSymbol, prioritySymbol, task.Title, depsStr, assigneeStr)
...
func printTaskDetail(task *TaskMeta) {
	fmt.Printf("ID:          %s\n", task.ID)
	fmt.Printf("Title:       %s\n", task.Title)
	fmt.Printf("Description: %s\n", task.Description)
	fmt.Printf("Status:      %s\n", task.Status)
