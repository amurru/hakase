...
**Voice replies (local Piper TTS):** enable `channels.telegram.text_to_speech` (`enabled: true`, `voice_path` pointing at a `.onnx` voice from HuggingFace `rhasspy/piper-voices`, plus `binary_path` — `pip install piper-tts` — and `ffmpeg_path`), then pick a mode with `/voice auto` (speak, get spoken answers), `/voice on` (always voice), or `/voice off` (default). In voice mode the answer arrives as one voice note instead of streamed text; synthesis hiccups fall back to the text answer. Multilingual: add a `voices` map (e.g. `{"de": ".../de_DE-thorsten-medium.onnx"}`) and replies **mirror the language whisper detected** on your voice note, falling back to the default voice otherwise. Env override: `HAKASE_TELEGRAM_TTS_ENABLED`. See [docs/telegram-voice/](docs/telegram-voice/).
