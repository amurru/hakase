...
**Voice dictation (local whisper.cpp):** the composer's mic button records locally, transcribes on the server, and drops the text into the input for you to review and edit before sending - same fully local pipeline as Telegram voice notes - your own host does the transcription, no third-party STT service sees the audio, and the clip is never stored (only the text). On a remote deployment the recording does travel from your browser to that host. Install ffmpeg + whisper.cpp to enable it; until then the button reports what is missing. Configure it under the top-level `speech_to_text` block. Each transcription is bounded by `speech_to_text.timeout_seconds` (default 180); the one-time model download has its own, larger `model_timeout_seconds` (default 1800) so a slow connection can still finish it. Env override: `HAKASE_STT_ENABLED`.
