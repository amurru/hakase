...
**Topics (one topic = one session):** `/topic` turns the DM into topic conversations — open topics with the ✚ / "All Messages" composer button, and each topic becomes its own session (auto-renamed to the session title; parallel runs across topics). The root area becomes a lobby: commands work, plain prompts get a hint. `/topic <session-id-prefix>` binds the current topic to an existing session, `/new` inside a topic resets it, and `/topic off` returns to a single conversation with bindings kept. Approvals and questions are delivered straight into the topic that started the run. Set `"pins": true` under `channels.telegram` to pin your prompt for the duration of each run.
