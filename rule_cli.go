// rule_cli.go - the `hakase rules` CLI: list and show project context files
// (AGENTS.md, with a project-scoped CLAUDE.md fallback).
//
// Mirrors the skill_cli.go conventions: subcommands parse with
// flag.ContinueOnError and return an int exit code (0 = success/help,
// 1 = runtime failure, 2 = usage error); the caller (main) decides whether
// to exit.
package main
