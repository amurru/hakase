...
- `speech_to_text` - local voice-note transcription via whisper.cpp, fully local (no cloud STT; issue #19, [docs/telegram-voice/](telegram-voice/spec.md)). Off unless explicitly enabled; disabled voice notes get an actionable setup hint instead of a run.
  ...
  - `model_timeout_seconds` - bound the first-use model download, which is a separate budget from `timeout_seconds`: the ggml file is tens to hundreds of MiB over the network, so bounding it by the transcription timeout would leave a slow connection unable to finish a first run. Default 1800. The web dictation handler fetches the model under this budget *before* starting the transcription budget, so `Transcribe`'s own model lookup short-circuits on the file already being present; the Telegram voice path still runs its download under `timeout_seconds`.
