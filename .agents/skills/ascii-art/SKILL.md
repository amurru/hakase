---
name: ascii-art
description: 'ASCII art generation with multiple tools: pyfiglet banners, cowsay, boxes decorations, image-to-ASCII conversion, web-sourced art, QR codes, weather art. Use when the user asks to create ASCII art, text banners, cow messages, decorative borders, or convert images to ASCII.'
license: MIT
metadata:
  author: 'hakase (ported via Hermes Agent; original Hermes skill by 0xbyt4, MIT)'
  version: 1.0.0
  source: https://github.com/NousResearch/hermes-agent/tree/main/skills/creative/ascii-art
---
...
```bash
python3 -m pyfiglet "HAKASE" -f slant | boxes -d stone
# Or without pyfiglet installed:
curl -s "https://asciified.thelicato.io/api/v2/ascii?text=HAKASE&font=Slant" | boxes -d stone
```
...
Search curated ASCII art from the web. Use `system_exec` with `curl`.
...
*Ported to hakase from [Hermes Agent](https://github.com/NousResearch/hermes-agent) (MIT). Original: Hermes Agent creative skill authored by 0xbyt4 (MIT). Uses OSS tools: pyfiglet, cowsay, boxes, TOIlet, ascii-image-converter (their own licenses).*
