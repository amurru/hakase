---
name: sketch
description: 'Throwaway HTML mockups: generate 2-3 design variants for head-to-head comparison. Use when the user wants to sketch a screen, see design directions, compare layouts, mockup a UI before building, explore visual options, or get quick interactive takes on a UI/UX idea. Each variant is a self-contained interactive HTML file.'
license: MIT
metadata:
  author: 'hakase (ported via Hermes Agent; adapted from GSD / Get Shit Done by Lex Christopherson, MIT)'
  version: 1.0.0
  source: https://github.com/NousResearch/hermes-agent/tree/main/skills/creative/sketch
---
...
**Verify variants -- either visually or via delegated browser check.** Don't just write HTML and hope it renders; confirm it looks right:

**Option A (preferred): delegate to web_researcher for a visual check.**
```
delegate_task(agent="web_researcher", description="Verify sketch renders", 
  prompt="Open file:///absolute/path/to/outputs/sketches/001-calm-editorial/index.html and check: (1) Does the layout look clean and readable? (2) Any visible bugs — overlapping text, unstyled elements, broken images? Report what you see.")
```

**Option B (manual): open the file in your browser.**
Tell the user: "Open `outputs/sketches/001-calm-editorial/index.html` in your browser and check for layout issues." If the user reports problems, fix and repeat.

Fix rendering issues before showing the user. Repeat for each variant.
...
If the user has an existing theme (colors, fonts, tokens), put shared tokens in `outputs/sketches/themes/tokens.css` and `@import` them in each variant. Keep tokens minimal:

```css
/* outputs/sketches/themes/tokens.css */
:root {
  --color-bg: #fafafa;
  --color-fg: #1a1a1a;
  --color-accent: #0066ff;
...
- Create `outputs/sketches/` (or `.planning/sketches/` if the user is using GSD conventions) in the repo root
- One subdir per variant: `NNN-stance-name/index.html` + `README.md`
- Tell the user how to open them: `open outputs/sketches/001-calm-editorial/index.html` on macOS, `xdg-open` on Linux, `start` on Windows
...
```
system_exec("mkdir -p outputs/sketches/001-calm-editorial")
write_file("outputs/sketches/001-calm-editorial/index.html", "<!doctype html>...")
write_file("outputs/sketches/001-calm-editorial/README.md", "## Variant: Calm editorial\n...")
# Verify: either delegate to web_researcher or tell user to open in browser
delegate_task(agent="web_researcher", description="Verify sketch variant",
  prompt="Open file://$(pwd)/outputs/sketches/001-calm-editorial/index.html and describe what you see. Any layout issues?")
```
