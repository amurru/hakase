---
name: p5js
description: 'p5.js sketches: generative art, shaders, interactive, 3D, data viz, animations, audio-reactive visuals, canvas-based creative coding. Use when the user asks for p5.js sketches, creative coding, generative art, interactive visualizations, canvas animations, browser-based visual art, data viz, shader effects, or any p5.js project.'
license: MIT
metadata:
  author: 'hakase (ported via Hermes Agent; skill by Hermes Agent, library p5.js by Lauren Lee McCarthy / Processing Foundation, LGPL-2.1)'
  version: 1.0.0
  source: https://github.com/NousResearch/hermes-agent/tree/main/skills/creative/p5js
---
...
For **interactive generative art** (seed exploration, parameter tuning): start from `templates/viewer.html`. Read the template first (use `read_file` on the bundled file at `.agents/skills/p5js/templates/viewer.html`), keep the fixed sections (seed nav, actions), replace the algorithm and parameter controls. This gives the user seed prev/next/random/jump, parameter sliders with live update, and PNG download — all wired up.
...
- For `loadImage()`/`loadFont()` from local files: use `scripts/serve.sh` or `system_exec` for `python3 -m http.server`
...
| **High-res PNG** | Puppeteer headless capture | `system_exec` for `node .agents/skills/p5js/scripts/export-frames.js sketch.html --width 3840 --height 2160 --frames 1` |
| **GIF** | `saveGif('output', 5)` — captures N seconds | Press 'g' to save |
| **Frame sequence** | `saveFrames('frame', 'png', 10, 30)` — 10s at 30fps | Then `system_exec` for `ffmpeg -i frame-%04d.png -c:v libx264 ./outputs/output.mp4` |
| **MP4** | Puppeteer frame capture + ffmpeg | `system_exec` for `bash .agents/skills/p5js/scripts/render.sh sketch.html ./outputs/output.mp4 --duration 30 --fps 30` |
...
The bundled `.agents/skills/p5js/scripts/export-frames.js` detects `_p5Ready` and calls `redraw()` once per capture for exact 1:1 frame correspondence. See `references/export-pipeline.md` § Deterministic Capture.

For multi-scene videos, use the per-clip architecture: one HTML per scene, render independently, stitch with `system_exec` for `ffmpeg -f concat`. See `references/export-pipeline.md` § Per-Clip Architecture.
...
2. **Open in browser** — `xdg-open sketch.html` (Linux) or the platform equivalent
3. **Local assets** (fonts, images) require a server: use `system_exec` for `python3 -m http.server 8080` in the project directory, then open `http://localhost:8080/sketch.html`
4. **Export PNG/GIF** — add `keyPressed()` shortcuts as shown above, tell the user which key to press
5. **Headless export** — use `system_exec` for `node .agents/skills/p5js/scripts/export-frames.js sketch.html --frames 300` for automated frame capture (sketch must use `noLoop()` + `_p5Ready`)
6. **MP4 rendering** — use `system_exec` for `bash .agents/skills/p5js/scripts/render.sh sketch.html ./outputs/output.mp4 --duration 30`
7. **Iterative refinement** — edit the HTML file, user refreshes browser to see changes
8. **Load references on demand** — use `read_file` on the bundled reference files (e.g., `read_file` on `.agents/skills/p5js/references/color-systems.md`) as needed during implementation
...
*Ported to hakase from [Hermes Agent](https://github.com/NousResearch/hermes-agent) (MIT). Skill by Hermes Agent (MIT). Underlying library: [p5.js](https://github.com/processing/p5.js) by Lauren Lee McCarthy / the Processing Foundation (LGPL-2.1).*
