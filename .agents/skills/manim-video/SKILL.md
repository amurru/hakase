---
name: manim-video
description: "Manim CE animations: create 3Blue1Brown-style math, algorithm, and concept explainer videos. Use when the user asks for animated explanations, math animations, concept visualizations, algorithm walkthroughs, technical explainers, 3Blue1Brown-style videos, equation derivations, architecture diagrams, or any programmatic animation with geometric or mathematical content. PLAN → CODE → RENDER → STITCH → AUDIO → REVIEW workflow across 7 modes."
license: MIT
metadata:
  author: 'hakase (ported via Hermes Agent; skill by Hermes Agent, library Manim CE by Grant Sanderson + Manim Community, MIT)'
  version: 1.0.0
  source: https://github.com/NousResearch/hermes-agent/tree/main/skills/creative/manim-video
---
...
Required: Python 3.10+, Manim Community Edition v0.20+ (`pip install manim`), LaTeX (`texlive-full` on Linux, `mactex` on macOS), and ffmpeg CLI on PATH. Verify with: `system_exec` to run `.agents/skills/manim-video/scripts/setup.sh`. Optional: ElevenLabs API key or Qwen3-TTS for narration voiceover. Reference docs tested against Manim CE v0.20.1.

Run all CLI tools via `system_exec`. Generated artifacts go to `./outputs/`.
...
Output final artifacts to `./outputs/`.
...
*Ported to hakase from [Hermes Agent](https://github.com/NousResearch/hermes-agent) (MIT). Skill by Hermes Agent (MIT). Underlying library: [Manim Community Edition](https://github.com/ManimCommunity/manim) by Grant Sanderson (3Blue1Brown) + the Manim Community (MIT).*
