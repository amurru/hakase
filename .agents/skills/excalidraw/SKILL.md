---
name: excalidraw
description: 'Create hand-drawn Excalidraw diagrams (architecture, flowcharts, sequence diagrams, concept maps) by writing standard Excalidraw element JSON as .excalidraw files. Use when the user asks for a diagram, flowchart, architecture diagram, sequence diagram, or concept map in Excalidraw format. Files can be opened at excalidraw.com or uploaded for shareable links.'
license: MIT
metadata:
  author: 'hakase (ported via Hermes Agent; skill by Hermes Agent, format by Excalidraw / Christopher Chedeau, MIT)'
  version: 1.0.0
  source: https://github.com/NousResearch/hermes-agent/tree/main/skills/creative/excalidraw
---
...
4. **Optionally upload** for a shareable link using `scripts/upload.py` via `system_exec`
...
Save to the `./outputs/` directory, e.g. `./outputs/my_diagram.excalidraw`.

### Uploading for a Shareable Link

Run the upload script (located in this skill's `scripts/` directory) via `system_exec`:

```bash
python .agents/skills/excalidraw/scripts/upload.py ./outputs/my_diagram.excalidraw
```
...
*Ported to hakase from [Hermes Agent](https://github.com/NousResearch/hermes-agent) (MIT). Skill by Hermes Agent (MIT). Underlying format: [excalidraw/excalidraw](https://github.com/excalidraw/excalidraw), created by Christopher Chedeau (Vjeux) / Excalidraw (MIT).*
