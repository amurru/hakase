---
name: ascii-video
description: "ASCII video: convert video, audio, or images to colored ASCII character video output (MP4, GIF, image sequence). Use when the user asks for ASCII video, text art video, terminal-style video, character art animation, retro text visualization, audio visualizer in ASCII, converting video to ASCII art, matrix-style effects, or any animated ASCII output. 6-stage pipeline across 6 modes: video-to-ASCII, audio-reactive, generative, hybrid, lyrics/text, and TTS narration. Per-clip parallel rendering via ffmpeg pipe."
license: MIT
metadata:
  author: 'hakase (ported from Hermes Agent; original Hermes creation, MIT)'
  version: 1.0.0
...
Required: Python 3.10+, NumPy, SciPy, Pillow (PIL), ffmpeg CLI on PATH, a monospace font (Menlo, Courier, Monaco, or auto-detected). Optional: OpenCV for video frame sampling/edge detection, ElevenLabs API key for TTS narration. No GPU required; all rendering is CPU-based.
...
*Ported to hakase from [Hermes Agent](https://github.com/NousResearch/hermes-agent) (MIT). Original Hermes Agent creation (MIT). Built for Hermes Agent; canonical source lives in the Hermes Agent repo.*
