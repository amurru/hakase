---
name: pretext
description: 'Build creative browser demos with DOM-free text layout using @chenglou/pretext. Use when the user asks for pretext demos, text flowing around shapes, ASCII-art effects using real prose, text-as-geometry games, kinetic typography with per-glyph physics, multiline shrink-wrap UI, or anything requiring line breaks before rendering.'
license: MIT
metadata:
  author: 'hakase (ported via Hermes Agent; skill by Hermes Agent, library @chenglou/pretext by Cheng Lou, MIT)'
  version: 1.0.0
...
This skill exists so hakase can make **cool demos** with it — the kind people post to X. See `pretext.cool` and `chenglou.me/pretext` for the community demo corpus.
...
2. **Start from a template**:
   ...
   - Use `write_file` to a new `.html` in `./outputs/` or the user's workspace.
...
5. **Verify locally**:
   Use `system_exec` to start a local server:
   ```sh
   cd <dir-with-html> && python3 -m http.server 8765
   ```
   Then open `http://localhost:8765/<file>.html` in a browser.
...
*Ported to hakase from [Hermes Agent](https://github.com/NousResearch/hermes-agent) (MIT). Skill by Hermes Agent (MIT). Underlying library: [@chenglou/pretext](https://github.com/chenglou/pretext) by Cheng Lou (MIT).*
