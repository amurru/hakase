...
description: 'Use when the user asks to follow the money, search public records, screen sanctions, perform corporate due diligence, resolve entities across sources, or build evidence chains. Investigative framework for public-records OSINT: government contracts, corporate filings, lobbying, sanctions, offshore leaks, property records, court records, web archives, and global news.'
license: MIT
metadata:
  author: 'Hermes Agent (adapted from ShinMegamiBoson/OpenPlanter, MIT)'
  version: 1.0.0
  source: https://github.com/NousResearch/hermes-agent/tree/main/optional-skills/research/osint-investigation
...
- general web research -> delegate to `web_researcher` sub-agent via `delegate_task`
...
The agent runs scripts via the `system_exec` tool. `SKILL_DIR` is the directory
...
python3 SKILL_DIR/scripts/fetch_opencorporates.py --query "Example Corp" \
    --jurisdiction us_ny --out outputs/opencorporates.csv
...
python3 SKILL_DIR/scripts/fetch_courtlistener.py --query "Smith v. Example Corp" \
    --type opinions --out outputs/courts.csv

# Wayback Machine - historical web captures
python3 SKILL_DIR/scripts/fetch_wayback.py --url "example.com" \
    --match host --collapse digest --out outputs/wayback.csv

# Wikipedia + Wikidata - narrative bio + structured facts
# Set HAKASE_OSINT_UA=your-app/1.0 (your@email) to identify yourself
...
python3 SKILL_DIR/scripts/fetch_gdelt.py --query '"Example Corp"' \
    --timespan 1y --out outputs/gdelt.csv
```
...
python3 SKILL_DIR/scripts/entity_resolution.py \
    --left  outputs/lobbying.csv   --left-name-col  client_name \
    --right outputs/contracts.csv  --right-name-col recipient_name \
    --out outputs/cross_links.csv
```
...
python3 SKILL_DIR/scripts/timing_analysis.py \
    --donations outputs/lobbying.csv --donation-date-col filing_date \
        --donation-amount-col income --donation-donor-col client_name \
        --donation-recipient-col registrant_name \
    --contracts outputs/contracts.csv --contract-date-col award_date \
        --contract-vendor-col recipient_name \
    --cross-links outputs/cross_links.csv \
    --permutations 1000 \
...
*Ported to hakase from [Hermes Agent](https://github.com/NousResearch/hermes-agent) (MIT). Original: OpenPlanter by ShinMegamiBoson (MIT).*
