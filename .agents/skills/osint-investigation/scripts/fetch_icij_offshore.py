...
$HAKASE_OSINT_CACHE/icij/ (default: ~/.cache/hakase-osint/icij/) and search
...
def _cache_dir() -> Path:
    base = os.environ.get("HAKASE_OSINT_CACHE")
    if base:
        return Path(base) / "icij"
    return Path.home() / ".cache" / "hakase-osint" / "icij"
...
def main() -> int:
    ...
    p.add_argument(
        ...
        type=Path,
        default=None,
        help="Override cache directory (default: $HAKASE_OSINT_CACHE/icij or ~/.cache/hakase-osint/icij)",
    )
    p.add_argument(
        "--force-refresh",
        action="store_true",
