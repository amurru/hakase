...
name: drug-discovery
description: >
  Use when the user asks about drug discovery, medicinal chemistry,
  pharmaceutical research, drug-likeness, Lipinski Ro5, QED, TPSA,
  molecular properties, ChEMBL bioactive compounds, OpenFDA drug
  interactions, ADMET profiles, lead optimization, PubChem compound
  search, OpenTargets gene-disease associations, adverse event lookup,
  or clinical pharmacology. Search bioactive compounds on ChEMBL,
  calculate drug-likeness (Lipinski Ro5, QED, TPSA, synthetic
  accessibility), look up drug-drug interactions via OpenFDA, interpret
  ADMET profiles, and assist with lead optimization in open-science
  drug research.
license: MIT
metadata:
  author: 'bennytimz (MIT); hakase ported via Hermes Agent (MIT)'
  version: 1.0.0
  source: https://github.com/NousResearch/hermes-agent/tree/main/optional-skills/research/drug-discovery
...
All curl/python3 one-liners below run via the `system_exec` tool.
...
Original: Drug Discovery & Pharmaceutical Research skill by bennytimz (MIT).*
