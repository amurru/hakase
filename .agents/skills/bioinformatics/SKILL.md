---
name: bioinformatics
description: 'Use when the user asks about bioinformatics, genomics, sequencing, variant calling, gene expression, single-cell analysis, protein structure, pharmacogenomics, metagenomics, phylogenetics, computational biology, RNA-seq, ChIP-seq, ATAC-seq, epigenomics, CRISPR, proteomics, population genetics, GWAS, genome assembly, structural biology, chemoinformatics, pathway analysis, immunoinformatics, metabolomics, systems biology, or any computational biology task. Gateway to 400+ genomics and computational biology skills from bioSkills and ClawBio open-source libraries.'
license: MIT
metadata:
  author: 'Hermes Agent (MIT)'
  version: 1.0.0
  source: https://github.com/NousResearch/hermes-agent/tree/main/optional-skills/research/bioinformatics
allowed-tools: read_file, write_file, patch, search_files, system_exec, python_interpreter, load_markdown_skill, delegate_task, vision
---
...
The git clone / cat / pip / conda / apt / brew / Rscript commands below run via the `system_exec` tool. Cloned repos are fetched on demand and never committed to the workspace.

**Sandbox note**: the git clone commands need outbound network access. Under hakase's `bubblewrap` sandbox mode, set `allow_network: true` in `config.json` under the `sandbox` block. Python packages installed via pip in the `.venv` also need `allow_pip_install: true`.
...
ClawBio:
  ...
  drug-photo -- Photo of medication to personalized PGx dosage card (via vision)
...
Original: Bioinformatics gateway skill by Hermes Agent (MIT).*
