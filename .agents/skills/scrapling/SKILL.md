---
name: scrapling
description: 'Use when the user asks to scrape static pages, JS-rendered pages, Cloudflare/anti-bot protected sites, spider crawling, stealth browsing, extract web data, bypass bot detection, or handle headless browser scraping. Wraps the Scrapling library with HTTP/Dynamic/Stealthy fetchers and a spider framework.'
license: MIT
metadata:
  author: 'FEUAZUR (MIT); library: D4Vinci/Scrapling (MIT); hakase ported via Hermes Agent (MIT)'
  version: 1.0.0
  source: https://github.com/NousResearch/hermes-agent/tree/main/optional-skills/research/scrapling
...
**Tool note:** Python examples run via the `python_interpreter` tool; `scrapling` CLI commands run via `system_exec`.

**Sandbox note:** The dynamic and stealth fetchers require outbound network access and launch a real browser subprocess. Under the `bubblewrap` sandbox mode, set `allow_network: true` in your sandbox configuration.
...
- When the `web_researcher` browser tools (Lightpanda) do not return the data you need

## Installation

Before using this skill, check that Scrapling is available and install it if missing:

```bash
python3 -c "import scrapling" 2>/dev/null || pip install "scrapling[all]"
```

If the import check fails, run the install commands:
...
result.items.to_json("./outputs/quotes.json")
```
...
*Ported to hakase from [Hermes Agent](https://github.com/NousResearch/hermes-agent) (MIT). Skill by FEUAZUR (MIT). Library: [D4Vinci/Scrapling](https://github.com/D4Vinci/Scrapling) (MIT).*
