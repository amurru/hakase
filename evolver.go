...
// evolveMutateFn is the model-backed mutation callback. Defaults to
// modelPromptFn so the engine works anywhere a model is available (agent
// runs, cron headless runs after cronModelBootstrap, CLI after bootstrap);
// tests override it with a deterministic fake.
var evolveMutateFn func(ctx context.Context, prompt string) (string, error) = modelPromptFn
