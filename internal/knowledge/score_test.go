...
func TestLoadBenchSet(t *testing.T) {
	...
	set, err := skill.LoadBenchSet(path)
	if err != nil {
		t.Fatalf("skill.LoadBenchSet: %v", err)
	}
	if len(set.Queries) != 1 || set.Queries[0].Query != "quantum" || set.Queries[0].K != 3 {
		t.Errorf("skill.LoadBenchSet parsed wrong: %+v", set)
	}
	...
	if _, err := skill.LoadBenchSet(dir + "/empty.json"); err == nil {
		t.Error("skill.LoadBenchSet accepted an empty set")
	}
}
