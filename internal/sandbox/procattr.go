...
// (procattr_pdeathsig.go / procattr_darwin.go / procattr_unix.go /
// procattr_windows.go) for packages that spawn hardened subprocesses but do
// not live in this package (internal/agent).

// ConfigureProcess applies platform process hardening to cmd before Start:
// process group + parent-death signal on Unix (process group only on darwin,
// which has no stdlib Pdeathsig), nothing on Windows (the Job Object is
// assigned after Start by AttachProcessTree).
