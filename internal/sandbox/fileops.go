...
func CreateFileOpsTools(log interfaces.LogFunc, sessionManager ExecSessionProvider, taskID string) ([]tool.Tool, error) {
	...
	if sessionManager != nil && taskID != "" {
		...
		if fos != nil {
			if f, ok := fos.(*fileOpsInfo); ok {
				sandboxRoot = f.RootDir
			}
		}
	}
	// read_file: read whole files or a line range.
	readTool, err := util.NewDocTool(functiontool.Config{
		Name:        "read_file",
	...
	writeTool, err := util.NewDocTool(functiontool.Config{
		Name:        "write_file",
	...
	patchTool, err := util.NewDocTool(functiontool.Config{
		Name:        "patch",
	...
	searchTool, err := util.NewDocTool(functiontool.Config{
		...
		return SearchFilesOutput{Matches: matches, Total: len(matches), Truncated: truncated, Context: safeSubdirHint(rootAbs)}, nil
	})
	if err != nil {
		return nil, err
	}
