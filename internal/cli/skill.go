// skill.go - the `hakase skill` CLI: create, list and validate markdown
// skills (SKILL.md).
//
...
// map to codes (0 = success/help, 1 = runtime failure, 2 = usage error).
package cli
...
func runSkillCreate(args []string) int {
	...
	if dir == "" {
		...
		dir = filepath.Join(skill.FindProjectRoot(cwd), ".agents", "skills")
	} else {
...
func runSkillList(args []string) int {
	...
	mdSkills := skill.DiscoverMarkdownSkills(cwd, skillDirs, nil)
...
func runSkillValidate(args []string) int {
	...
	if _, err := skill.ParseMarkdownSkill(path); err != nil {
...
func runSkillEvolve(args []string) int {
	...
	report, err := skill.RunEvolutionPass(opts)
	if err != nil {
