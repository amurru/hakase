...
func runCronJob(job CronJob, log hakaseagent.LogFunc) {
	...
	subAgentTools, subAgentToolsets := hakaseagent.BuildSubAgentTools("default", parentEnv, subAgentLogFunc)
	subAgentTools = hakaseagent.FilterBlockedTools(subAgentTools)

	genCfg := hakaseagent.BuildGenerationConfig("")
	...
	for {
		...
		for ev, runErr := range subRunner.Run(guardCtx, "cron_scheduler", taskID, msg, agent.RunConfig{}) {
			if runErr != nil {
				if hakaseagent.IsToolCallJSONErr(runErr) && attempt < hakaseagent.MaxToolCallRepairAttempts {
					util.DebugWarn("tool_call_repair", "cron_job", job.ID, "attempt", attempt+1, "error", runErr)
					msg = hakaseagent.ToolCallRepairMessage(runErr, attempt)
					attempt++
					repaired = true
					break
				}
	...
	if silent {
		...
		notifyCronJob("failed", job.ID, job.Name, hakaseagent.Truncate(summaryText, 300), outputPath)
	} else {
		notifyCronJob("completed", job.ID, job.Name, hakaseagent.Truncate(summaryText, 300), outputPath)
	}
}
...
func runNativeCronJob(job CronJob, log hakaseagent.LogFunc) {
	...
	case "evolve":
		...
		report, err := skill.RunEvolutionPass(skill.EvolutionOptions{
			SkillsDir:  "./skills",
			Mutate:     true,
...
func resolveCronSkills(names []string, log hakaseagent.LogFunc) ([]skill.MarkdownSkill, map[string]bool, error) {
	cwd, _ := os.Getwd()
	all := skill.DiscoverMarkdownSkills(cwd, currentConfig.SkillDirs, log)

	byName := make(map[string]skill.MarkdownSkill, len(all))
	for _, s := range all {
		key := strings.ToLower(s.Frontmatter.Name)
		byName[key] = s
	}

	var resolved []skill.MarkdownSkill
...
func cronModelBootstrap() error {
	...
	// MCP manager is needed by hakaseagent.BuildSubAgentTools for the "default" agent
