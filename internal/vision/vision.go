...
// VisionModelLLM is the legacy-path model created from cfg.VisionModel in
// setupRunner. nil = not configured or creation failed. Set by root.
var VisionModelLLM model.LLM

// CurrentModelInfo returns the main model's capabilities. Set by root via
// sandbox_hooks_init.go. nil = fetch not complete/failed.
var CurrentModelInfo func() *interfaces.ModelInfo

// CurrentConfig returns the active runtime configuration. Set by root via
// sandbox_hooks_init.go.
var CurrentConfig func() *config.Config

// currentModelInfo calls the hook; nil-safe.
func currentModelInfo() *interfaces.ModelInfo {
...
// currentConfig calls the hook; nil-safe.
...
func VisionModelUsable(cfg *config.Config) bool {
	if VisionModelLLM != nil {
		return true
	}
...
func ResolveMainModelVision(mi *interfaces.ModelInfo, cfg *config.Config) VisionMode {
	if cfg == nil {
	...
	// config.Config override (highest precedence).
...
func VisionHandler(ctx agent.Context, input VisionInput) (VisionOutput, error) {
	...
	mi := currentModelInfo()
	cfg := currentConfig()
	mode := ResolveMainModelVision(mi, cfg)
...
func RewriteAttachedImages(ctx gocontext.Context, req *model.LLMRequest, mode VisionMode) {
...
func DescribeOrWarnImage(ctx gocontext.Context, data []byte, mime, question string, mode VisionMode) string {
...
// paths (sandbox-aware).
func ResolveImageSource(ctx gocontext.Context, imageURL string) ([]byte, string, error) {
	switch {
...
func describeImageWithVisionModel(ctx gocontext.Context, img []byte, mime string, question string) (string, error) {
	llm := VisionModelLLM
	if llm == nil {
