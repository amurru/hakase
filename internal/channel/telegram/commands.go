...
func (b *Bot) cmdSessions(ctx context.Context, c conv, m *models.Message) {
	...
	bound := b.store.Get().Chats[chatKey(c.chatID)].SessionID
	n := len(summaries)
	if n > 8 {
		n = 8
	}
...
func (b *Bot) cmdUse(ctx context.Context, c conv, m *models.Message, args string) {
	...
	switch {
	case len(matches) == 0:
		b.sendText(ctx, c, "No session matches <code>"+prefix+"</code> — see /sessions.", nil, false)
	case len(matches) > 1:
		b.sendText(ctx, c, "Prefix is ambiguous ("+fmt.Sprint(len(matches))+" matches) — use more characters.", nil, false)
	default:
		id := matches[0]
		if err := b.store.Update(func(s *state.State) error {
			if s.Chats == nil {
