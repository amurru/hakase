...
// chats to notify and how to render them. Gate prompts carry the session id
// so a transport can route the prompt to the conversation that started the
// run (an empty id means the origin is unknown).
type PushHandler interface {
	// ApprovalPrompt asks the user to approve or deny a pending tool call.
	ApprovalPrompt(sessionID, id, tool, risk, reason, command string)
	// ClarifyPrompt asks the user to answer a mid-task question, optionally
	// from a list of choices (multiSelect allows several).
	ClarifyPrompt(sessionID, id, question string, choices []string, multiSelect bool)
