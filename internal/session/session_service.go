...
func (s *SessionService) GetActiveSession() (*Session, error) {
	...
	if SeedHintedContextPathsHook != nil {
		SeedHintedContextPathsHook(session.HintedContextFiles)
	}
	return session, nil
}
...
// SaveSession persists a session to the store. This is exported for use
// by context management (HistoryBuilder) which modifies session messages
// directly and needs to save without going through AddMessage.
func (s *SessionService) SaveSession(session *Session) error {
	return s.store.Save(session)
}

// Store returns the underlying SessionStore.
func (s *SessionService) Store() *SessionStore {
	return s.store
}
