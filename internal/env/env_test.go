...
func TestSystemEnvEnabled(t *testing.T) {
	...
	if !config.SystemEnvEnabled(nil) {
		t.Error("config.SystemEnvEnabled(nil) should default to enabled")
	}
	cfg := &config.Config{}
...
func TestSystemEnvMaxChars(t *testing.T) {
	if got := config.SystemEnvMaxChars(nil); got != DefaultSystemEnvMaxChars {
		t.Errorf("config.SystemEnvMaxChars(nil) = %d, want default %d", got, DefaultSystemEnvMaxChars)
	}
	cfg := &config.Config{SystemEnv: config.SystemEnvConfig{MaxChars: 400}}
	if got := config.SystemEnvMaxChars(cfg); got != 400 {
		t.Errorf("config.SystemEnvMaxChars(cfg) = %d, want 400", got)
	}
}

func TestBuildEnvironmentReminderBubblewrapNote(t *testing.T) {
	info := sampleSystemInfo()
	info.ExecSandbox = string(sandbox.SandboxModeBubblewrap)
	block := BuildEnvironmentReminder(info, 0)
	if !strings.Contains(block, "bubblewrap sandbox") {
		t.Errorf("bubblewrap mode should add exec note:\n%s", block)
	}
	// paths/off mode carries no note.
	info.ExecSandbox = string(sandbox.SandboxModePaths)
	if strings.Contains(BuildEnvironmentReminder(info, 0), "bubblewrap sandbox") {
		t.Error("paths mode should not add bubblewrap note")
	}
	info.ExecSandbox = string(sandbox.SandboxModeOff)
	if strings.Contains(BuildEnvironmentReminder(info, 0), "bubblewrap sandbox") {
...
// contextBlockFor filters a block string by agent name against an apply-to list.
// Mirrors the root-level function in agent.go (which cannot be imported from package main).
