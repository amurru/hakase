...
// truncateContextBlock so the block can never bloat the prompt.
...
func DetectSystemInfo(cwd string, log LogFunc) *SystemInfo {
	...
	// setupRunner (sandbox.CurrentSandbox). A bubblewrap sandbox isolates subprocesses
	...
	if sandbox.CurrentSandbox != nil {
		info.ExecSandbox = string(sandbox.CurrentSandbox.Mode)
	}
...
func BuildEnvironmentReminder(info *SystemInfo, maxChars int) string {
	...
	if info.ExecSandbox == string(sandbox.SandboxModeBubblewrap) {
	...
	return truncateContextBlock(block, maxChars)
}

// truncateContextBlock caps content at maxChars using a 70%% head / 20%% tail
// split with a marker in the middle. Mirrors truncateContextFile from
// instruction_context.go. maxChars <= 0 falls back to DefaultSystemEnvMaxChars.
func truncateContextBlock(content string, maxChars int) string {
	if maxChars <= 0 {
		maxChars = DefaultSystemEnvMaxChars
	}
