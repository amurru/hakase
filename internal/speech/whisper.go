...
// WAV → whisper-cli → read the transcript file. All work happens in a temp
// directory that is removed on return; audio bytes never leave the machine.
// mime is accepted for interface stability but deliberately unused — ffmpeg
// detects the container from the bytes, so no remote-declared value ever
// reaches a filename or argv.
func (w *WhisperCLI) Transcribe(ctx context.Context, audio []byte, mime string, durationSec int) (string, error) {
	if err := ctx.Err(); err != nil {
		return "", err
	}
	if w.cfg.MaxSeconds > 0 && durationSec > w.cfg.MaxSeconds {
		return "", fmt.Errorf("%w: %ds > max_seconds %d", ErrTooLong, durationSec, w.cfg.MaxSeconds)
	}

	dir, err := os.MkdirTemp("", "hakase-asr-")
	if err != nil {
		return "", fmt.Errorf("speech: temp dir: %w", err)
	}
	defer os.RemoveAll(dir)

	inPath := filepath.Join(dir, "input.audio")
	if err := os.WriteFile(inPath, audio, 0600); err != nil {
		return "", fmt.Errorf("speech: write input: %w", err)
	}
	...
	if out, err := util.CombinedOutputContext(ctx, ffcmd); err != nil {
		return "", fmt.Errorf("speech: ffmpeg decode: %w: %s", err, tailRunOutput(string(out)))
	}
	...
	// is always passed and the argv stays fully literal.
	...
	whcmd := &exec.Cmd{Path: whbin, Args: []string{whbin,
		"-f", wavPath, "-m", modelPath, "-np", "-otxt", "-of", base,
		"-l", w.cfg.Language,
	}}
	if out, err := util.CombinedOutputContext(ctx, whcmd); err != nil {
		return "", fmt.Errorf("speech: whisper-cli: %w: %s", err, tailRunOutput(string(out)))
	}

	raw, err := os.ReadFile(base + ".txt")
	if err != nil {
		return "", fmt.Errorf("speech: whisper transcript missing: %w", err)
	}
