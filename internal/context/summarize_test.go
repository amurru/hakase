...
func TestBuildSummarizePromptSkipsToolTranscripts(t *testing.T) {
	msgs := []session.Message{
		{Role: "agent", Content: "tool call data", InContext: true, Kind: session.MessageKindToolCall},
...
func TestBuildSummarizePromptCapsAtTail(t *testing.T) {
	...
	for i := 0; i < 30; i++ {
		msgs = append(msgs, session.Message{
			Role: "user", Content: strings.Repeat("x", 10000), InContext: true, Kind: session.MessageKindText,
		})
	}
