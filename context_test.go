...
func newTestBuilder(t *testing.T) (*HistoryBuilder, *SessionService) {
	t.Helper()
	...
	b.SetModelInfo(&ModelInfo{ContextWindow: 100_000, MaxInputTokens: 90_000})
	return b, svc
}
...
func TestFitToBudgetNoTruncationUnderTrigger(t *testing.T) {
	...
	history := b.buildHistory(session.Messages, "")
	before := len(history)
	history = b.fitToBudget(session, history, []*genai.Content{genai.NewContentFromText("new", genai.RoleUser)}, &ModelInfo{ContextWindow: 1_000_000})
...
func TestFitToBudgetStageBSnip(t *testing.T) {
	...
	b.SetModelInfo(&ModelInfo{ContextWindow: 40_000})
	...
	history := b.buildHistory(session.Messages, "")
	trimmed := b.fitToBudget(session, history, []*genai.Content{genai.NewContentFromText("new", genai.RoleUser)}, &ModelInfo{ContextWindow: 40_000})
	...
	if int64(EstimateContentsTokens(trimmed)) > target {
		t.Fatalf("trimmed history %d tokens exceeds target %d", EstimateContentsTokens(trimmed), target)
	}
...
func TestStageATrimsToolResultsOnly(t *testing.T) {
	...
	session.AddMessageWithMeta("agent", "tool output here", "", 5000, MessageKindToolResult)
	session.AddMessageWithMeta("user", "q2", "", 5, MessageKindText)
